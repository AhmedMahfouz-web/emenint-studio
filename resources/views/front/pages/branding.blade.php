@extends('layouts.front')

@section('content')
    <div id="page">
        <section class="svg-header svg-header--project">
            <div class="svgh-bh">
                <div id="svg-animaiotn" class="inview flex flex--bottom">
                    <div class="js-parallaxheader">
                        <svg class="deaw studio" id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 383.19 391.63">
                            <defs>
                                <style>
                                    .cls-1 {
                                        fill: none;
                                        stroke: #2c2a2a;
                                        stroke-miterlimit: 10;
                                        stroke-width: 2px;
                                    }
                                </style>
                            </defs>
                            <path class="cls-1"
                                d="m336.44,233.26c-15.09-1.71-64.12-21.03-101.51-31.75-8.64-2.46-16.62-4.47-23.45-5.7,6.83-1.23,14.82-3.24,23.45-5.7,37.42-10.65,86.45-29.97,101.51-31.75,17.26-2.15,32.54-6.03,45.56-11.66-1.99-8.11-4.63-16.1-7.82-23.96-5.7-7.22-10.86-14.93-15.57-23.08-4.63-8.02-8.61-16.32-11.87-24.77-5.37-6.96-11.16-13.41-17.3-19.39-11.42,8.39-22.38,19.75-32.98,33.59-9.03,12.22-50.28,45.02-78.25,72.04-6.45,6.25-12.18,12.16-16.67,17.46,2.35-6.53,4.6-14.45,6.79-23.16,9.49-37.73,17.27-89.86,23.26-103.78,6.77-16.03,11.05-31.2,12.68-45.28-8.01-2.33-16.26-4.05-24.66-5.21-9.1,1.32-18.36,1.94-27.77,1.94s-18.44-.7-27.38-2.1c-8.71,1.17-17.19,2.96-25.44,5.29,1.56,14.08,5.91,29.25,12.6,45.36,6.07,13.93,13.85,66.05,23.26,103.78,2.19,8.71,4.44,16.63,6.79,23.16-4.49-5.3-10.22-11.21-16.67-17.46-27.93-27.09-69.18-59.88-78.25-72.04-10.49-13.88-21.49-25.17-32.87-33.62-6.03,5.77-11.63,12.06-16.84,18.75-3.41,8.54-7.5,16.87-12.2,25.02-4.63,8.02-9.82,15.62-15.51,22.67-3.35,8.13-6.04,16.37-8.14,24.68,12.97,5.69,28.29,9.5,45.58,11.76,15.09,1.71,64.12,21.03,101.51,31.74,8.64,2.46,16.62,4.47,23.46,5.7-6.83,1.23-14.82,3.24-23.45,5.7-37.42,10.65-86.45,29.97-101.51,31.75-17.26,2.15-32.54,6.03-45.56,11.66,1.99,8.11,4.63,16.11,7.82,23.96,5.7,7.22,10.86,14.93,15.57,23.08,4.63,8.02,8.61,16.32,11.87,24.77,5.37,6.96,11.16,13.41,17.3,19.39,11.42-8.39,22.37-19.75,32.98-33.59,9.03-12.22,50.28-45.02,78.25-72.04,6.45-6.25,12.18-12.16,16.67-17.46-2.35,6.53-4.6,14.45-6.79,23.16-9.49,37.73-17.27,89.85-23.26,103.78-6.77,16.03-11.05,31.2-12.68,45.28,8.01,2.33,16.26,4.05,24.66,5.21,9.1-1.32,18.36-1.95,27.77-1.95s18.44.7,27.39,2.1c8.71-1.17,17.19-2.95,25.44-5.29-1.56-14.08-5.91-29.25-12.6-45.36-6.07-13.93-13.85-66.05-23.26-103.78-2.19-8.71-4.44-16.63-6.79-23.16,4.49,5.3,10.22,11.21,16.67,17.46,27.93,27.09,69.18,59.88,78.25,72.04,10.49,13.88,21.5,25.17,32.87,33.62,6.03-5.77,11.63-12.06,16.84-18.75,3.41-8.54,7.5-16.87,12.2-25.02,4.63-8.02,9.82-15.62,15.51-22.67,3.35-8.13,6.04-16.37,8.14-24.68-12.97-5.69-28.29-9.5-45.58-11.76Z" />
                        </svg>
                    </div>
                </div>
                <div class="grid-width svgh-bh flex flex--bottom text-center">
                    <div>
                        <div class="subline inview inview--up">(Welcome to the show)</div>
                        <h1 class="inview inview--up">Branding</h1>
                    </div>
                </div>
            </div>
        </section>

        <section class="info-swiper">
            <div class="grid-width">
                <div class="swiper-container info-swiper-content swiper-branding">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide">
                            <div class="inview inview--up">
                                Brand Name Suggestion </div>

                            <div class="editor-content copy-s inview inview--up">
                                We assist in selecting a powerful and emorable brand name that reflects your business
                                identity and resonates
                                with your target market. </div>

                        </div>
                        <div class="swiper-slide">
                            <div class="inview inview--up">
                                Brand Guidelines </div>

                            <div class="editor-content copy-s inview inview--up">
                                We create brand guidelines that serve as a roadmap
                                for maintaining brand consistency and ensuring all communications align
                                with your brand's voice and image. </div>

                        </div>
                        <div class="swiper-slide">
                            <div class="inview inview--up">
                                Brand Identity </div>

                            <div class="editor-content copy-s inview inview--up">
                                We design a cohesive brand identity system, including
                                color palettes, typography, and visual elements, to establish a strong and
                                recognizable brand presence.
                            </div>

                        </div>
                        <div class="swiper-slide">
                            <div class="inview inview--up">
                                Brand Strategy </div>

                            <div class="editor-content copy-s inview inview--up">
                                We develop a comprehensive brand strategy that
                                defines your brand's positioning, values, and personality, ensuring
                                consistency across all touch points. </div>

                        </div>
                        <div class="swiper-slide">
                            <div class="inview inview--up">
                                Logo Concept and Design </div>

                            <div class="editor-content copy-s inview inview--up">
                                Our talented designers create unique and
                                visually appealing logo concepts that convey your brand's essence and
                                differentiate it from competitors </div>

                        </div>
                    </div>
                    <div class="swiper-scrollbar"></div>
                </div>
            </div>
        </section>

        <section class="separator inview inview--up">
            <div class="grid-width copy-s">
                <div class="js-seperator">
                    <div class="separator--text flex">
                        <div class="flex--strech">
                            <span class="top"><span>Sec.</span>(01)</span>
                            <span class="bottom"><span>Sec.</span>(02)</span>
                        </div>
                        <div>
                            <span class="top">Text</span>
                            <span class="bottom">Text</span>
                        </div>
                    </div>
                    <div class="separator--line inview inview--line"></div>
                </div>
            </div>
        </section>


        <section>
            <div class="grid-width project-wrapper">
                <a class="js-projectlink inview inview--up" href="{{ route('plaza_gardens') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/plaza_gardens/20.jpg') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('zenda') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/zenda/1.jpg') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('mountain_gate') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/mountain_gate/1.jpg') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('profit') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/profit/1.jpg') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('esomo') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/esomo/1.jpg') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('oracal') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/oracal/3.jpg') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('loja') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/loja/1.jpg') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('artal') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/artal/1.png') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('regal') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/regal/1.png') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('mentor') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/mentor/8.jpg') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('ouda_w_sala') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/Ouda_w_sala/2.jpg') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
                <a class="js-projectlink inview inview--up" href="{{ route('mawish_elmasey') }}">
                    <div class="scale">
                        <div class="project-image overflow-hidden">
                            <div class="parallax-section inview2">
                                <div class="parallax-section--image cover-image lazyload"
                                    data-bg="{{ asset('images/Mawish_elmasey/1.jpg') }}">
                                </div>
                            </div>
                        </div>
                    </div>
                </a>
            </div>
        </section>

    </div><!-- /page -->
@endsection
